...
from schemas.const import TAIL_LEN, MAX_ATTEMPTS
...
async def add_study(config: StudyConfigRequest, conn: AsyncSession):
    ...
    try:
        ...
        for _ in range(MAX_ATTEMPTS):
            try:
                study_config = StudyConfiguration(study_id=study_id)
                ...
                if study_config is not None:
                    conn.expunge(study_config)
        else:
            raise HTTPException(
                500,
                detail="Unable to create unique UUID. Please try again. If this problem continues please contact an adminstrator for help.",
            )
        ...
        await conn.commit()
        study_code = str(study_config.id)[
            -TAIL_LEN:
        ]  # TODO: Change to Study ID and update all calls for a study ID
        return study_code
...
async def save_user_survey(
    ...
    conn.add(UserSurveyConfig(id=survey_id, study_config_id=study_id))


async def save_file_uploads(
    study_id: uuid.UUID, files: FileUploadsRequest, conn: AsyncSession
):
